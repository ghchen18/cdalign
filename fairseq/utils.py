...
def extract_att_input_alignment(sample, model, src_punc_tokens, alignment_task=None):
    ...
    _, attn = model.decoder(prev_output_tokens, encoder_out=encoder_out)

    if alignment_task in ['eamout']:
        attn = attn['EAM_attn']
    else:
        attn = attn['attn']  ###  bsz x tgt_len x src_len
    alignments = {}  ## alignments are 0-index 
    for idx in range(bsz):
        sample_id = sampleID[idx]
        alignments[sample_id] = []
        tgt_idxs = ((tgt_sent[idx] != pad) & (tgt_sent[idx] != eos)).nonzero().squeeze(dim=-1) 
        tgt_valid = (tgt_sent[idx] != pad).nonzero().squeeze(dim=-1)  
        ...
        if len(tgt_valid) != 0 and len(src_invalid) < len(src_sent[idx]):  
            ...
            if src_sent[idx,-2] in src_punc_tokens:
                if tgt_sent[idx,tgt_valid[-2]] in src_punc_tokens:
...
def extract_att_output_alignment(sample, model, src_punc_tokens, alignment_task=None):
    src_sent = sample['net_input']['src_tokens']
    src_lengths = sample['net_input']['src_lengths']
    prev_output_tokens =  sample['net_input']['prev_output_tokens']
    tgt_sent = sample['target']
    ...
    _, attn = model.decoder.extract_features(prev_output_tokens, encoder_out=encoder_out, alignment_heads=alignment_heads)
