...
def add_preprocess_args(parser):
    ...
    group.add_argument("--process-only-alignment", action="store_true",
                       help="Only process the given alignment data")
    # fmt: on
    return parser
...
def add_dataset_args(parser, train=False, gen=False):
    ...
    group.add_argument('--dataset-impl', metavar='FORMAT',
                        choices=get_available_dataset_impl(),
                        help='output dataset implementation')
    group.add_argument('--alignment-task', default='vanilla', type=str, choices=['vanilla', 'attin','attout','eamout'],
                            help='Use different methods for constrained decoding.')
    group.add_argument('--beam', default=5, type=int, metavar='N', help='beam size')
    group.add_argument('--alignment-layer', default=4, type=int,
                            help='Decoder layer used to extract attention weights for alignments.')
    group.add_argument("--set-shift", action="store_true",
                       help="set to use ATT-Input method")
    ...
    if gen:
        ...
        group.add_argument('--print-vanilla-alignment', action="store_true",
                           help='Set to extract alignment during inference.')
        parser.add_argument('--trg_pt',metavar='FILE',
                        help='path(s) to target constraints file')
