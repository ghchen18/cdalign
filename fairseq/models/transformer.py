...
class TransformerDecoder(FairseqIncrementalDecoder): #
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, src_dict=None):
        ...
        self.alignment_layer = [getattr(args, 'alignment_layer', 4)]
        self.set_EAM = True if args.alignment_task in ['eamout'] else False 
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            TransformerDecoderLayer(args, no_encoder_attn, add_EAM=( self.set_EAM  and idx in self.alignment_layer))
        ...
        **unused,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.decoder_layerdrop):               
                x, layer_attn, layer_EAM_attn = layer(
                    x,
                    encoder_state,
                    ...
                    need_attn= (idx in self.alignment_layer),
                    need_head_weights=(idx in self.alignment_layer),
                )
                inner_states.append(x)
                if layer_attn is not None and (idx in self.alignment_layer):
                    attn = layer_attn.float()
                if layer_EAM_attn is not None and (idx in self.alignment_layer):
                    EAM_attn = layer_EAM_attn.float()
