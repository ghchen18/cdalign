...
def load_vanilla_checkpoint(args, trainer, **passthrough_args):
    """
    Load a checkpoint and restore the training iterator.
    ...
    *passthrough_args* will be passed through to
    ...
    if args.distributed_rank == 0:
        os.makedirs(args.save_dir, exist_ok=True)
    checkpoint_path = os.path.join(args.save_dir, "checkpoint_last_f.pt")

    f_state = torch.load(checkpoint_path, map_location=lambda s, l: default_restore_location(s, 'cpu'))
    model_f_dict = trainer.model.state_dict()
    base_dict = {k:v for k,v in f_state['model'].items() if k in model_f_dict}
    model_f_dict.update(base_dict)
    trainer.model.load_state_dict(model_f_dict)
    ...
    print("Successfully load pretrained vanilla transformer and frozen its params.")
