...
class LabelSmoothedCrossEntropyCriterionWithAlignment(LabelSmoothedCrossEntropyCriterion):
    ...
    def __init__(self, args, task):
        ...
        self.args = args
        self.set_shift = args.set_shift if hasattr(args, "set_shift") else False
        self.pad = 1
        self.eos = 2
    ...
    def add_args(parser):
        ...
        parser.add_argument('--alignment-lambda', default=0.3, type=float, metavar='D',
                            help='weight for the alignment loss') ##0.05

    def forward(self, model, sample, reduce=True):
        """Compute the loss for the given sample.
        fairseq/criterions/label_smoothed_cross_entropy_with_alignment.py:44
        ...
        if alignment_loss is not None:
            ...
            logging_output['nll_loss'] = utils.item(loss.data) if reduce else loss.data

        return loss, sample_size, logging_output

    def compute_alignment_loss(self, sample, net_output):
        if self.args.alignment_task in ['eamout']:
            attn_prob = net_output[1]['EAM_attn']
        else:
        ...
        if len(align) > 0:
            if not self.set_shift:
                ...
                loss = -((attn[align[:, 1][:, None]+1, align[:, 0][:, None]]).log() * align_weights[:, None]).sum()
        else:
            return None
