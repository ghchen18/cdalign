...
class SequenceAttbeam(object):
    def __init__(
        ...
        diverse_beam_groups=-1, diverse_beam_strength=0.5, args=None,
    ):
        self.models = models
        ...
        self.tgt_punc = [2,3,4]
        for w in range(len(tgt_dict)):
            if tgt_dict[w] in punc:
                self.tgt_punc += [w]

        self.src_punc = [2,3,4] ## mask special symbols
        for w in range(len(src_dict)):
            if src_dict[w] in punc:
                self.src_punc += [w]
    ...
    def prepare_pdic(self, phrases, src_tokens, target=None):
        count=1 ## used to count the number of constrained phrases
        p_dict={}  ## used to store the constraints, key:constrain_ID, value: constraint token
        p_dict[0]=[] ## constraint ID 0 is set as null

        ## used to mask the constraint ID for each source token, if it is not a constraint, the value is set as 0.
        ps_mask = torch.zeros_like(src_tokens[0]) 
        for item in phrases:
            if 'tgt_span' in item:
                p=[]
                w=item['tgt_span']
                for i in range(w[1]):
                    if w[0]+i < len(target[0]):
                        p += [target[0,w[0]+i].cpu().item()]
                p_dict[count]=p
                w = item['src_span']
                ps_mask[int(w[0]):int(w[0])+int(w[1])] = count
                count += 1
            else:
                src_p = self.tgt_dict.encode_line(item['src_phrase'], add_if_not_exist=False, append_eos=False).type_as(src_tokens)
                tgt_p = self.tgt_dict.encode_line(item['tgt_phrase'], add_if_not_exist=False, append_eos=False).type_as(src_tokens)
                if len(src_p)==0 or len(tgt_p)==0:
                    continue
                src_p_len = len(src_p)
                for idx in range(len(src_tokens[0]) - src_p_len):
                    if src_p.equal(src_tokens[0, idx:idx+src_p_len]):
                        if all(ps_mask[idx:idx+src_p_len]==0):
                            p_dict[count]  = tgt_p
                            ps_mask[idx:idx+src_p_len] = count
                            count += 1
                            break

        return p_dict, ps_mask, count-1

    def generate_batched_itr(
        self, data_itr, beam_size=None, maxlen_a=0.0, maxlen_b=None,
        cuda=False, timer=None, prefix_size=0, trg_pt=None,
        tgt_dict=None,
    ):
        ...
        for sample in data_itr:
            ...
            with torch.no_grad():
                ...
                if timer is not None:
                    timer.start()
                use_cons=False
                if sample_id in trg_pt and len(trg_pt[sample_id])>0:                                    
                    phrase_dic, ps_mask, pnum = self.prepare_pdic(trg_pt[sample_id], src_tokens, target=sample['target'])
                    if pnum > 0:
                        use_cons=True
                        if self.args.set_shift:
                            hypo = self.att_input_decoding(encoder_input, phrase_dic, pnum, ps_mask, maxlen_r=2.0, beam_size=beam_size)  
                        else:
                            hypo = self.att_output_decoding(encoder_input, phrase_dic, pnum, ps_mask, maxlen_r=2.0, beam_size=beam_size)                 
                        if timer is not None:
                ...
                if not use_cons:
                    ...
                    if timer is not None:
                        timer.stop(hypos[0][0]['tokens'].size(0)-1)
                    yield sample['id'], encoder_input['src_tokens'], sample['target'], hypos[0][0]['tokens']

 
    def att_output_decoding(self, encoder_input, pdic, pnum, ps_mask, maxlen_r=1.5, beam_size=1):
        src_tokens = encoder_input['src_tokens']
        bsz, srclen = src_tokens.size()
        maxlen = min(int(maxlen_r * srclen), 150)
        ...
        encoder_outs = []
        incremental_states = {}
        phrase_yet = src_tokens.new_zeros(bsz*beam_size, pnum).byte() # whether each phrase has already in trans
        phrase_in = src_tokens.new_zeros(bsz*beam_size) # 0: trans not in a phrase; >0: the phrase trans 1
        phrase_in_count = src_tokens.new_zeros(bsz*beam_size) # position of trans in current phrase (start from 0)
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                phrase_yet = phrase_yet.index_select(0,reorder_state)
                phrase_in = phrase_in.index_select(0,reorder_state)
                phrase_in_count = phrase_in_count.index_select(0,reorder_state)

            lprobs, avg_attn_scores = self._decode(tokens[:, :step + 1], encoder_outs, incremental_states)
            src_invalid = torch.tensor([ x in [self.pad,self.eos] for x in src_tokens[0]]).nonzero().transpose(0,1)
            avg_attn_scores[:,src_invalid] = 0
            index = avg_attn_scores.argmax(-1) # attention index
            for idx,pos in enumerate(index):
                if ps_mask[pos] != 0 and phrase_in[idx] == 0: # aligned to a source constrain
                    unfinish_mark = 1 - phrase_yet[idx,ps_mask[pos]-1] # the aligned constrain is not in the trans yet
                    if unfinish_mark:
                        phrase_yet[idx,ps_mask[pos]-1] = 1
                        phrase_in[idx] = ps_mask[pos]
                        phrase_in_count[idx]=0
                if phrase_in[idx] > 0: # the hypo is in a phrase 
                    tmp_pos=pdic[phrase_in[idx].item()][phrase_in_count[idx].item()]
                    tmp = lprobs[idx,:].max().clone() # maximum lprob over the vocab
                    lprobs[idx,:] = -math.inf # reset the output distribution
                    lprobs[idx,tmp_pos] = tmp
                    if len(pdic[phrase_in[idx].item()])-1 == phrase_in_count[idx]:
                        phrase_in_count[idx] = 0
                        phrase_in[idx] = 0 # finish this phrase
                    else:
                        phrase_in_count[idx] += 1
    ...
    def att_input_decoding(self, encoder_input, pdic, pnum, ps_mask, maxlen_r=1.5, beam_size=1):
        """Constrained decoding with Att-Input.
        Args:
            encoder_input (dict): dictionary containing the inputs to
                    *model.encoder.forward*.
            pdic (dict): dictionary containing the target constraints: constraint id: target token list
            pnum (int): number of constraint pairs
            ps_mask (LongTensor): the constraint id for each token in src_tokens, 0 indicates the source token is not in any constraint 
            max_len_r (int, optional): to determine maximum length of the generated sequence
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if step < maxlen:    
                ...
                phrase_yet_align = phrase_yet.index_select(0,indices_sel)
                ...
                for idx in range(align_k*beam_size): # modify tokens_align, phrase_yet_align, phrase_in_align and phrase_in_count_align accordingly
                    if phrase_in_align[idx] == 0 and ps_mask[index_src[idx]] != 0 and indices_align[idx] != self.eos: # align to a constraint and not in a unfinished cons
                        ...
                        if unfinish_mark:
                            ...
                            phrase_in_count_align[idx] = 0
                            tokens_align[idx,step+1] = pdic[phrase_in_align[idx].item()][0] # revise the last token of the hypo if it aligns to a new constraint
                    if phrase_in_align[idx] > 0: # aligned to a constraint
                        if len(pdic[phrase_in_align[idx].item()])-1 == phrase_in_count_align[idx]: # finish the constraint
                            phrase_in_count_align[idx] = 0
                            phrase_in_align[idx] = 0 
                        else:
                            phrase_in_count_align[idx] += 1 # move to the next constraint position
                
                tmp, idx_align=torch.unique(tokens_align[:,:step+2],dim=0,return_inverse=True) # index of the non-repeating hypos
                idx_align_new = tmp.data.new(tmp.size(0)).fill_(500)
                count = 0
                for i in range(idx_align.size(0)):
                    w = idx_align[i]
                    if i < idx_align_new[w]:
                        idx_align_new[w] = i
                        count += 1
                        if count == tmp.size(0): # repeaing hypos appear in a score descending order; therefore we keep the one with the highest score
                            break
                idx_align = idx_align_new
                ...
                cand_scores = scores_align.index_select(0,idx_final).view(1,-1)
                cand_beams = torch.div(idx_final,align_k).view(1,-1)
                cand_indices = tokens_align[:,step+1].index_select(0,idx_final).view(1,-1)
                phrase_yet_align = phrase_yet_align.index_select(0,idx_final)
                phrase_in_align = phrase_in_align.index_select(0,idx_final)
                phrase_in_count_align = phrase_in_count_align.index_select(0,idx_final)
                if fsb < beam_size:
                    cand_scores[fsb:] = -math.inf
            else:
                # make probs contain cumulative scores for each hypothesis
                lprobs.add_(scores[:, step - 1].unsqueeze(-1))
            ...
            phrase_yet = phrase_yet_align.index_select(0,active_hypos[0])
            phrase_in = phrase_in_align.index_select(0,active_hypos[0])
            phrase_in_count = phrase_in_count_align.index_select(0,active_hypos[0])
            # swap buffers
            tokens, tokens_buf = tokens_buf, tokens
            scores, scores_buf = scores_buf, scores
            if attn is not None:
    ...
    def _decode(self, tokens, encoder_outs, incremental_states=None, high_attn=False):
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs):
        with torch.no_grad():
            if incremental_states is not None and incremental_states[model] is not None:
                decoder_out = list(model.decoder(tokens, encoder_out, incremental_state=incremental_states[model]))
            else:
                decoder_out = list(model.decoder(tokens, encoder_out))
            decoder_out[0] = decoder_out[0][:, -1, :]
            attn = decoder_out[1]
            if self.args.alignment_task in ['eamout']:
                attn = attn.get('EAM_attn',None)
            else:
